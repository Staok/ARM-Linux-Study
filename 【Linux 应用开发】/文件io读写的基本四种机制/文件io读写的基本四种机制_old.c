 ...
 文件io的读写的基本四种机制：
    非阻塞（查询），阻塞（休眠死等-唤醒返回），poll/select（设置阻塞事件和时间），异步通知
 ...
 poll/select（设置阻塞事件和时间）
    ...
    当 poll/select 返回时候，根据 返回事件 revents 判断，再进行 读写，否则返回是 超时 或者 错误。
        ...
        在休眠期间，如果有人操作了硬件，驱动程序获得数据后就会把APP唤醒，导致poll或select立刻返回；
        如果在“超时时间”内无人操作硬件，则时间到后poll或select函数也会返回。
    ...
    关于 poll 更详细的说明：https://blog.csdn.net/skypeng57/article/details/82743681
    更多好的参考：https://blog.csdn.net/a987073381/article/details/52295690
 
 
 异步通知
    主要是使用信号机制。
    app 向 一个具体的驱动程序 注册 IO事件（SIGIO）信号和 处理/回调/钩子/handler 函数，并告知自己的进程号，并使能 异步通知
        一个具体的驱动程序 在 特定 情况下 通过 内核提供的 API 向 APP 发 IO事件（SIGIO）信号，
    ...
    信号注册 API：void (* signal( int signo, void (*func)(int) )  )(int);
        ...
        返回值类型   ：void (*)(int);
        signal 函数详解：https://www.cnblogs.com/nufangrensheng/p/3514547.html
    ...
    下面的情况可以产生Signal：引自 https://blog.csdn.net/xiufu004/article/details/7198223
        1. 按下CTRL+C产生SIGINT。
        2. 硬件中断，如除0，非法内存访问（SIGSEV）等等。
        3. Kill() 函数可以对进程发送 Signal。
        4. "Kill -s <信号> <进程号>" 命令。实际上是对 Kill() 函数的一个包装。
